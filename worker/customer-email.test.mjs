...
for(const variant of ['shipping','missing-address','seller']){const msg=customerReviewMessage(env.ORDER_EMAIL_FROM,variant);assert.deepEqual(msg.to,['nathanagellatly@gmail.com']);assert(msg.html.includes('REVIEW SAMPLE ONLY'));assert(msg.html.includes('https://aliviastreasuredthreads.com/images/brand/logo.jpg'));assert(!msg.html.includes('Maddie'));assert(!msg.html.includes('\u2014'));await sendCustomerReview(env,variant);assert.deepEqual(calls.at(-1).to,['nathanagellatly@gmail.com']);}

...
db.exec(await readFile(new URL('./customer-email-schema.sql', import.meta.url), 'utf8'));
const ORDERS = {
  prepare(sql) {
    let args = [];
    const query = {
...
const env = { ORDERS, ORDER_EMAIL_ENABLED: 'true', CUSTOMER_EMAIL_ENABLED: 'true', CUSTOMER_EMAIL_START_AT: '1', PAYMENTS_MODE: 'test', STRIPE_SECRET_KEY: 'sk_test_fixture', STRIPE_WEBHOOK_SECRET: 'whsec_fixture', SHIPPING_RATE_CENTS: '0', CATALOG_URL: 'https://catalog.test/site.json' };
...
try {
  ...
  assert.equal(db.prepare('SELECT count(*) AS n FROM customer_email_outbox').get().n, 0);
  ...
  assert.equal(db.prepare('SELECT count(*) AS n FROM customer_email_outbox').get().n, 1);
